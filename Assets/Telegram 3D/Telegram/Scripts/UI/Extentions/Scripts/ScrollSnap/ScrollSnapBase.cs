...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IScrollSnap
    {
        ...
        [Serializable] public class SelectionChangeStartEvent : UnityEvent { }
        [Serializable] public class SelectionPageChangedEvent : UnityEvent<int> { }
        [Serializable] public class SelectionChangeEndEvent : UnityEvent<int> { }

        [Tooltip("The screen / page to start the control on\n*Note, this is a 0 indexed array")]
        [SerializeField] public int StartingScreen = 0;
        ...
        {
            int childCount = ScreensContainer.childCount;
        ...
        internal void UpdateVisible()
        {
            ...
            _maskSize = IsVertical ? MaskArea.rect.height : MaskArea.rect.width;
            HalfNoVisibleItems = (int)Math.Round(_maskSize / (ChildSize * MaskBuffer), MidpointRounding.AwayFromZero) / 2;
            _bottomItem = _topItem = 0;

            for (int i = HalfNoVisibleItems + 1; i > 0; i--)
            {
        ...
        {
            return IsVertical ?
                (int)Math.Round((ScrollStartPosition - pos.y) / ChildSize) :
                (int)Math.Round((ScrollStartPosition - pos.x) / ChildSize);
        }
        ...
        {
            _childPos = -ChildSize * page;
            if (IsVertical)
            {
                target.y = _childPos + ScrollStartPosition;
            }
            else
            {
                target.x = _childPos + ScrollStartPosition;
            }
        }
